...
class MediaWikiRenderer(BaseRenderer):
    """ Renderer for MediaWiki formatted documents """
    ...
    def cacheFilenames(self, node):
        # We need to prevent file splitting for this renderer so we'll
        # make that adjustment here

        # Using the side-effect of the filename property
        try:
            setattr(node, "splitlevel", -1000)
    ...
    def do_flushleft(self, node): #TBD
    ...
    def do_array(self, node, cellspacing=(2,1), render=unicode): #TBD
    ...
    def do_cline(self, node): #TBD
        return ''
        
    def do_multicolumn(self, node): #TBD
        return unicode(node)

    # Bibliography
    def do_thebibliography(self, node): #TBD
        output = ['', 'Bibliography', '']
        for item in node:
    ...
    def do_bibliographystyle(self, node): #TBD
        return u''

    def do_bibliography(self, node): #TBD
        return self.default(node)
  
    def do_cite(self, node): #TBD
        output = []
        for item in node.citation():
            output.append(unicode(item))
        return u''.join(output)

    def do_bibliographyref(self, node): #TBD
        return self.default(node)
    ...
    def do_linebreak(self, node): #TBD
        return u'\n\n'
    ...
    def do_ref(self, node): #TBD
        return unicode(node.idref['label'].ref)

    def do_pageref(self, node): #TBD
        return u'*'

    def do_label(self, node): #TBD
        return u''

    # Floats
    
    def do_figure(self, node): #TBD
        return unicode(node)
        
    do_table = do_marginpar = do_figure

    def do_caption(self, node): #TBD
    ...
    def do_symbol(self, node): #TBD
        return u'*'

    # Footnotes
    def do_footnote(self, node): #TBD
    ...
    def do_footnotetext(self, node): #TBD
        self.do_footnote(self, node)
        return ''
    
    def do_footnotemark(self, node): #TBD
        return u'[%s]' % (len(self.footnotes)+1)
    
    # Index
    
    def do_theindex(self, node): #TBD
        return u''
    ...
    def do_itemize(self, node): #tbd (sublists)
        output = []
        for item in node:
            output.append(("* %s"%item.childrenSource).strip())
        return self.addBlock(u'\n'.join(output))
        
    def do_enumerate(self, node): #tbd (sublists)
        output = []
        for item in node:
            output.append(("# %s"%item.childrenSource).strip())
        return self.addBlock(u'\n'.join(output))
        
    def do_description(self, node): #TBD
        output = []
        for item in node:
    ...
    def do_egroup(self, node): #TBD
        return u''
    
    # Pictures
    
    def do_picture(self, node): #TBD
        return u''
        
    # Primitives
    
    def do_par(self, node): #TBD
    ...
    def do__superscript(self, node): #TBD
        return self.default(node)
    
    def do__subscript(self, node): #TBD
        return self.default(node)
    
    # Quotations
    
    def do_quote(self, node): #TBD
        backslash = self['\\']
    ...
    def do_maketitle(self, node): #TBD
        output = dict(title = "", author = "", date = "", thanks = "")
...
"""%output
    ...
    def do_section(self, node): 
        return u'==%s==\n%s'%(self.fill(node.fullTitle),node)
    ...
    do_subparagraph = do_subsubparagraph = do_paragraph

    def do_title(self, node): #TBD
        return u''
        
    do_author = do_date = do_thanks = do_title
    
    def do_abstract(self, node): #TBD
        return self.center(node)

    # Sentences

    def do__dollar(self, node): #TBD
        return u'$'
        
    def do__percent(self, node): #TBD
        return u'%'
        
    def do__opencurly(self, node): #TBD
        return u'{'
        
    def do__closecurly(self, node): #TBD
        return u'}'
    
    def do__underscore(self, node): #TBD
        return u'_'
        
    def do__ampersand(self, node): #TBD
        return u'&'
        
    def do__hashmark(self, node): #TBD
        return u'#'
    
    def do__space(self, node): #TBD
        return u' '

    def do_LaTeX(self, node): #TBD
        return u'LaTeX'

    def do_TeX(self, node): #TBD
        return u'TeX'

    def do_emph(self, node): #TBD
        return self.default(node)
        
    do_em = do_emph

    def do__tilde(self, node): #TBD
        return u' '

    def do_enspace(self, node): #TBD
        return u' '

    do_quad = do_qquad = do_enspace
        
    def do_enskip(self, node): #TBD
        return u''

    do_thinspace = do_enskip

    def do_underbar(self, node): #TBD
        return self.default(node)

    # Space
    
    def do_hspace(self, node): #TBD
        return u' '
    
    def do_vspace(self, node): #TBD
        return u''
    ...
    def do_verbatim(self, node): #TBD
        return re.sub(r'^\s*\n', r'', unicode(node)).rstrip()

    do_alltt = do_verbatim

    def do_mbox(self, node): #TBD
        return self.default(node)

    def do__at(self, node): #TBD
        return u''

    def do__backslash(self, node): #TBD
        return u'\\'
